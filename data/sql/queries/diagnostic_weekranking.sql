-- Show 10 sample failing rows with expected vs actual
...
SELECT
    ...
    wr.ranking_year,
    wr.ranking_week,
    wr.total_points AS actual,
    COALESCE(b.expected_points, 0) AS expected,
    COALESCE(b.num_results_used, 0) AS results_in_window
FROM WeeklyRanking wr
LEFT JOIN best4 b
...
ORDER BY wr.ranking_year, wr.ranking_week, wr.player_id
LIMIT 10;
